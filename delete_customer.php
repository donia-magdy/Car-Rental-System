...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
        ...
        $check_customer_query = "SELECT * FROM user WHERE ssn = ?";
        $stmt = $mysqli->prepare($check_customer_query);
        ...
        if ($result->num_rows > 0) {
            ...
            $delete_query = "DELETE FROM user WHERE ssn = ?";
            $stmt = $mysqli->prepare($delete_query);
            $stmt->bind_param("s", $ssn);

            if ($stmt->execute()) {
                echo '<script>alert("Customer deleted successfully.");</script>';
                echo"<script>window.location.href=\"delete_customer.php\"</script>";
            } else {
...
<body>
     ...
     <header id="header" class="alt">
      ...
      <form id="deleteForm" method="post">
        ...
        <label for="ssn">SSN Customer:</label>
        <input type="text" name="ssn" required><br>
        <input type="submit" value="Delete Customer" class="delete-car-btn" name="action" onclick="return confirm('Are you sure you want to delete this customer?');">
    </form>
