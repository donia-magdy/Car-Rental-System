...
<head>
    <meta charset="UTF-8">
    <title>Reporting Interface - Car Reservations</title>
    <meta
      name="viewport"
...
<body>
 ...
 <header id="header" class="alt">
    ...
    <h1>Reservations of a Car within a Period</h1>
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" onsubmit="return validateReservationForm();">
        ...
        <div id="car_selection">
            <label for="car_id">Car Plate ID:</label>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["generate_report"])) {
        ...
        // SQL query to fetch all reservations of a specific car within a specified period including all car information
      $sql = "SELECT r.*, c.*, u.* 
        FROM reservation r
        JOIN car c ON r.plate_id = c.plate_id
        JOIN user u ON r.ssn = u.ssn
        WHERE  r.plate_id = '$selected_car_id' AND ((pickup_time <= '$pickup_time' AND return_time >= '$pickup_time')
...
";
        ...
        if ($result && $result->num_rows > 0) {
            ...
            echo "<th>Reservation Number</th><th>Car Information</th></tr>";
          ...
          echo "<script>window.location.href=\"car_reservations.php\"</script>";
