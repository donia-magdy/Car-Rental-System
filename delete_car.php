...
<?php
   if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['action'])) {
        ...
        $check_car_query = "SELECT * FROM car WHERE plate_id = ?";
        $stmt = $mysqli->prepare($check_car_query);
        $stmt->bind_param("s", $car_plate_id);
        $stmt->execute();
        $result = $stmt->get_result();
    
        if ($result->num_rows > 0) {
            // Reservation exists, proceed with deletion
            $delete_query = "DELETE FROM car WHERE plate_id = ?";
            $stmt = $mysqli->prepare($delete_query);
            $stmt->bind_param("s", $car_plate_id);
    
            if ($stmt->execute()) {
                echo '<script>alert("car deleted successfully.");</script>';
                echo"<script>window.location.href=\"delete_car.php\"</script>";
            } else {
...
<body>
    ...
    <h1>Delete Car</h1>
    <label for="plate_id">Car Plate ID:</label>
    <input type="text" name="plate_id" required><br>
    <input type="submit" value="Delete Car" class="delete-car-btn" name="action"  onclick="return confirm('Are you sure you want to delete this car?');">
